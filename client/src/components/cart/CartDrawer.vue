          ...
          <div class="pointer-events-auto w-screen max-w-md">
            <div class="flex h-full flex-col overflow-y-scroll bg-white shadow-xl dark:bg-gray-800">
                    ...
                    <ul role="list" class="-my-6 divide-y divide-gray-200 dark:divide-gray-700">
                      <li v-for="item in cartStore.items" :key="item.productId" class="flex py-6">
                        <div class="h-24 w-24 flex-shrink-0 overflow-hidden rounded-md border border-gray-200 dark:border-gray-700">
                          <img :src="item.imageUrl || 'https://placehold.co/600x400'" :alt="item.name" class="h-full w-full object-cover object-center">
                        </div>
                        ...
                        <div class="ml-4 flex flex-1 flex-col">
                          ...
                          <div class="flex flex-1 items-end justify-between text-sm">
                            <div class="flex items-center gap-2 border rounded px-2 py-1 dark:border-gray-600">
                                <button @click="cartStore.updateQuantity(item.productId, item.quantity - 1)" class="text-gray-500 hover:text-gray-700 dark:text-gray-400">-</button>
                                <p class="text-gray-500 dark:text-gray-300 w-4 text-center">{{ item.quantity }}</p>
                                <button @click="cartStore.updateQuantity(item.productId, item.quantity + 1)" class="text-gray-500 hover:text-gray-700 dark:text-gray-400">+</button>
                            </div>

                            <div class="flex">
                              <button type="button" class="font-medium text-indigo-600 hover:text-indigo-500 dark:text-indigo-400" @click="cartStore.removeItem(item.productId)">Eliminar</button>
                            </div>
                          </div>
                        </div>
                      </li>
              ...
              <div class="border-t border-gray-200 px-4 py-6 sm:px-6 dark:border-gray-700">
                ...
                <p class="mt-0.5 text-sm text-gray-500 dark:text-gray-400">Envío e impuestos calculados al finalizar.</p>
                <div class="mt-6">
                ...
                <div class="mt-6 flex justify-center text-center text-sm text-gray-500">
                  <p>
                    o
                    <button type="button" class="font-medium text-indigo-600 hover:text-indigo-500 dark:text-indigo-400" @click="cartStore.closeCart()">
                      Continuar Comprando
